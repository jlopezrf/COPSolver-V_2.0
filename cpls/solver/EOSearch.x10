...
public class EOSearch extends SingleSolHeuristic{
 	...
 	public def this(){
 		...
 		if(super.heuristicParams instanceof EOParameters){
 			...
 			this.pdfUserSel = params.getPdfUserSel();//opts("--RoTS_tabu_duration", -1.0);
 			this.selSecond = params.getSelSecond();//opts("--RoTS_aspiration", -1.0);
 	...
 	protected def search() : Long{
 		...
 		Console.OUT.print("Costo (EOSearch): " + currentCost + ". Con variables: ");
 		displaySolution();
 		Console.OUT.print("\n");
 		return currentCost;
 	}
